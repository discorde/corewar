...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2013/12/08 14:34:48 by lcaminon          #+#    #+#             */
/*   Updated: 2013/12/08 14:39:34 by lcaminon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static ssize_t	add_line(char **line, ssize_t *size, t_buff *buff, char delim)
{
	char		*tmp;
	ssize_t		i;

	i = buff->i + 1;
	while (i < buff->size && buff->buff[i] != delim)
		++i;
	...
	while (++i < *size)
		tmp[i] = (*line)[i];
	while (++(buff->i) < buff->size && buff->buff[buff->i] != delim)
	...
	*line = tmp;
	*size = i;
	if (buff->i < buff->size && buff->buff[buff->i] == delim)
		return (1);
	return (0);
}

int					get_delim(int const fd, char **line, char delim)
{
	...
	*line = NULL;
	if ((buff = get_buff(&head_buff, fd)) == NULL)
		return (-1);
	size = 0;
	while ((ret = add_line(line, &size, buff, delim)) == 0)
	{
